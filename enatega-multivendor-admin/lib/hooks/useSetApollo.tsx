...
export const useSetupApollo = (): ApolloClient<NormalizedCacheObject> => {
  ...
  const normalizedServerUrl = SERVER_URL?.endsWith('/')
    ? SERVER_URL
    : `${SERVER_URL ?? ''}/`;
  const normalizedWsServerUrl = WS_SERVER_URL?.endsWith('/')
    ? WS_SERVER_URL
    : `${WS_SERVER_URL ?? ''}/`;
  ...
  const httpLink = createHttpLink({
    uri: `${normalizedServerUrl}graphql`,
  });

  // WebSocketLink with error handling
  const wsLink = isBrowser
    ? new WebSocketLink(
      new SubscriptionClient(`${normalizedWsServerUrl}graphql`, {
        reconnect: true,
        timeout: 30000,
        lazy: true,
      })
  ...
  const request = async (operation: Operation): Promise<void> => {
    const data = isBrowser ? localStorage.getItem(`user-${APP_NAME}`) : null;
    ...
    if (operationName !== 'MetricsGeneral' && shouldRefreshToken()) {
      await fetchMetricsToken(normalizedServerUrl);
    }
