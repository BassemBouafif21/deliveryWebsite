  ...
  function onCompleted({ ownerLogin }: IOwnerLoginDataResponse) {
    if (!ownerLogin?.token) {
      showToast({
        type: 'error',
        title: 'Login',
        message: 'Login response is invalid. Please try again.',
      });
      return;
    }

    const rawUserType = String(ownerLogin?.userType || '').toUpperCase();
    const normalizedUserType =
      rawUserType === 'ADMIN' ||
      rawUserType === 'STAFF' ||
      rawUserType === 'VENDOR' ||
      rawUserType === 'RESTAURANT'
        ? rawUserType
        : 'ADMIN';

    const normalizedOwnerLogin = {
      ...ownerLogin,
      userType: normalizedUserType as
        | 'ADMIN'
        | 'STAFF'
        | 'VENDOR'
        | 'RESTAURANT',
      restaurants: Array.isArray(ownerLogin?.restaurants)
        ? ownerLogin.restaurants
        : [],
      permissions: Array.isArray(ownerLogin?.permissions)
        ? ownerLogin.permissions
        : [],
    };
    ...
    onUseLocalStorage(
      'save',
      `user-${APP_NAME}`,
      JSON.stringify(normalizedOwnerLogin)
    );
    setUser(normalizedOwnerLogin);
    const redirect_url =
      DEFAULT_ROUTES[normalizedUserType] || DEFAULT_ROUTES.ADMIN;
